...
public:  
  OSimProcess( const char* modelName, int index )
  {
    ...
    model.updVisualizer().updSimbodyVisualizer().addSlider( "Input Force " + std::to_string( index + 1 ) , inputSliderID, -1.0, 1.0, 0.0 );
    feedbackSliderID = index * 2 + 2;
    model.updVisualizer().updSimbodyVisualizer().addSlider( "Feedback Force " + std::to_string( index + 1 ), feedbackSliderID, -2.0, 2.0, 0.0 );
  ...
  static void Update( OSimProcess* process )
  {
    ...
    while( process->isUpdating )
    {
      ...
      while( process->model.updVisualizer().updInputSilo().isAnyUserInput() ) 
      {
        int sliderID;
        SimTK::Real forceValue;
        while( process->model.updVisualizer().updInputSilo().takeSliderMove( sliderID, forceValue ) )
        {
          std::cout << "force from slider " << sliderID << ": " << forceValue << std::endl;
          if( sliderID == process->inputSliderID ) process->inputActuator->setOverrideActuation( process->state, forceValue );
          if( sliderID == process->feedbackSliderID ) process->feedbackActuator->setOverrideActuation( process->state, forceValue );
        }
      }
